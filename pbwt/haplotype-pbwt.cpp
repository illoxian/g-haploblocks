...
public:
    ...
    LR_file_hap(string filename,int_all buffer_size,bool visible = false){
        ...
        int BufferSize_big = 4096, BufferSize = buffer_size;
...
public:
    ...
    int_all get_index(int_all i,bool principal){
        return choice^principal ? second_array[i] : first_array[i];
    }

    void set_index(int_all i,int_all val,bool principal){
        choice^principal ? second_array[i] = val : first_array[i] = val;
    }
